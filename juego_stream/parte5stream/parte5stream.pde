...
void setup() {
  ...
  powerUpPrueba = new PowerUp(2, 30, 500, 250, 8, "Balas x2");
...
void juego() {
  ...
  if (enemigo != null) {
    ...
    if (enemigo.vidas < 0) enemigo = null;
  }
...
void actualizarBalas() {
  ...
  for (Bala bala : balas) {
    bala.movimientoBala();
    bala.limiteVertical();
    if (enemigo != null)enemigo.colision(bala);
...
void menu() {
  ...
  if (botones[0] == null) {
    ...
    botones[0] = new Boton(width/2, 200, "Jugar", 40);
    botones[1] = new Boton(width/2, 300, "Controles", 40);
    botones[2] = new Boton(width/2, 400, "Historia", 40);
    botones[3] = new Boton(width/2, 500, "Puntajes", 40);
  }

  for (int i = 0; i < botones.length; i++) {
    botones[i].dibujarBoton();
    if(botones[i].texto == "Jugar" && botones[i].clickeado) estado = "juego";
  }
}
